        ...
        <tbody className="text-gray-700">
          {
            enrolledCourses.map((course, index)=>(
              <tr key={index} className="border-b border-gray-500/20">
                ...
                <td className="px-4 p-3 max-sm:hidden">
                  {calculateCourseDuration(course)}
                </td>
                <td className="px-4 p-3 max-sm:hidden">{progressArray[index]&& `${progressArray[index].lectureCompleted}/${progressArray[index].totalLectures}`} <span>Lectures</span></td>
                <td className="px-4 p-3 max-sm:text-right">
                  <button className="px-3 sm:px-5 py-1.5 sm:py-2 max-sm:text-xs bg-blue-600 text-white rounded"
