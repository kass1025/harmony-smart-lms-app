    ...
    setChapters(
      chapters.map((chapter)=>{
        if(chapter.chapterId === currentChapterId){
          const newLecture = {
            ...lectureDetails,
            lectureDetails: chapter.chapterContent.length>0 ? 
      ...
      <form onSubmit={handleSubmit} className='flex flex-col gap-4 max-w-md w-full text-gray-500'>
        ...
        <div className='flex items-center justify-between flex-wrap'>
          <div className='flex flex-col gap-1'>
            ...
            className='outline-none md:py-2.5 py-2 w-32 px-3 rounded border border-gray-500' required />
          </div>
          <div className='flex md:flex-row flex-col items-center gap-3'>
            <p>Course Thumbnail</p>
            <label htmlFor="thumbnailImage">
              <img src={assets.file_upload_icon} alt="" className='p-3 text-blue-500 rounded' />
        ...
        <div>
          {// If the chapter div is collapsed add chapters
            chapters.map((chapter, chapterIndex)=>(
              <div key={chapterIndex} className='bg-white border rounded-lg mb-4'>
                <div className='flex items-center justify-between p-4 pb-4'>
                  ...
                  <img src={assets.cross_icon} alt="" className='cursor-pointer text-red-600' onClick={()=>handleChapter('remove', chapter.chapterId)} />
                </div>
                {
                  // If the chapter box is not collapsed add lectures in the chapter content
                  !chapter.collapsed && 
                  (
                    <div className='p-4'>
                      {
                        chapter.chapterContent.map((lecture, lectureIndex)=>(
                          <div key={lectureIndex} className='flex items-center justify-between mb-2'>
                            ...
                            <img src={assets.cross_icon} alt="" className='cursor-pointer bg-red-600 text-red-800' onClick={()=>handleLecture('remove', chapter.chapterId, chapterIndex)} />
                          </div>
                        ))}
        ...
        {/* Add or submit button to add all the course data from the form */}
